function Invoke-EgmMembershipChange {
    ...
    $results = foreach ($identifier in $userIdentifiers) {
        ...
        try {
            ...
            Write-PtLog "User not found: $identifier" 'ERROR' $LogPath
            [PSCustomObject]$row
            continue
        }
        ...
        if ($Action -eq 'Add') {
            if ($isMember) {
                ...
                Write-PtLog "Skipped (already a member): $target" 'INFO' $LogPath
            }
            elseif ($PSCmdlet.ShouldProcess($target, 'Add to group')) {
                try {
                    ...
                    $row.Result = 'Success'
                    Write-PtLog "Added: $target" 'INFO' $LogPath
                }
                catch {
                    $row.Result = 'Failed'; $row.Detail = $_.Exception.Message
                    Write-PtLog "Failed to add: $target - $($_.Exception.Message)" 'ERROR' $LogPath
                }
            }
        }
        else {
            if (-not $isMember) {
                ...
                Write-PtLog "Skipped (not a member): $target" 'INFO' $LogPath
            }
            elseif ($PSCmdlet.ShouldProcess($target, 'Remove from group')) {
                try {
                    ...
                    $row.Result = 'Success'
                    Write-PtLog "Removed: $target" 'INFO' $LogPath
                }
                catch {
                    $row.Result = 'Failed'; $row.Detail = $_.Exception.Message
                    Write-PtLog "Failed to remove: $target - $($_.Exception.Message)" 'ERROR' $LogPath
                }
            }
        }
    ...
    Write-PtLog "---- Summary ----" 'INFO' $LogPath
    $results | Group-Object Result | ForEach-Object { Write-PtLog "$($_.Name): $($_.Count)" 'INFO' $LogPath }
    if ($LogPath) { Write-PtLog "Log written to: $LogPath" 'INFO' $LogPath }
