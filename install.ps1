<#
.SYNOPSIS
    Install the PowerToolbox PowerShell module for the current user.
...
.DESCRIPTION
    Runs three ways:
      ...
      ./install.ps1 -Source ./PowerToolbox (from any folder holding the module)
...
.PARAMETER Source
    Install from this folder instead of downloading. Must contain PowerToolbox.psd1.
...
$moduleName = 'PowerToolbox'
$tempRoot   = $null

# Duplicated as PowerToolbox/Private/Get-PtInstallPath.ps1: this script cannot import the
# module it is installing, so the logic lives in both places. Keep the two in sync.
# Keep this file pure ASCII - Windows PowerShell 5.1 reads a BOM-less UTF-8 file as ANSI.
function Get-PtInstallRoot {
...
try {
    ...
    if ($Source) {
        ...
        if (-not (Test-Path (Join-Path $sourceModule 'PowerToolbox.psd1'))) {
            throw "-Source folder does not contain PowerToolbox.psd1: $sourceModule"
        }
        $sourceKind = 'path'
    }
    # An explicit -Version means "install that release", so it outranks the working tree.
    elseif (-not $Version -and $PSScriptRoot -and (Test-Path (Join-Path (Join-Path $PSScriptRoot $moduleName) 'PowerToolbox.psd1'))) {
        ...
        if ($Version) {
            $url        = "https://github.com/$repo/releases/download/$Version/PowerToolbox.zip"
            $notFound   = "No release found (tag $Version)."
        }
        else {
            $url        = "https://github.com/$repo/releases/latest/download/PowerToolbox.zip"
        ...
        $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("PowerToolbox-" + [Guid]::NewGuid().ToString('N'))
        New-Item -ItemType Directory -Path $tempRoot -Force | Out-Null

        $zipPath = Join-Path $tempRoot 'PowerToolbox.zip'
        ...
        $found = Get-ChildItem -Path $extractRoot -Recurse -Filter 'PowerToolbox.psd1' -File |
            Select-Object -First 1
        if (-not $found) {
            throw "Downloaded archive does not contain PowerToolbox.psd1."
        }
        $sourceModule = $found.Directory.FullName
    }

    # ---- Install ----------------------------------------------------------------------
    $installRoot = Get-PtInstallRoot
    ...
    if (-not $KeepLegacy) {
        foreach ($legacy in @('CalendarPermissions', 'EntraGroupMembers')) {
            ...
            if (Test-Path $legacyPath) {
                ...
                Write-Host "Removed superseded module $legacy (its commands are now in PowerToolbox)." -ForegroundColor Yellow
            }
        }
    }
    ...
    $installedManifest = Join-Path $destination 'PowerToolbox.psd1'
    try {
    ...
    Write-Host "PowerToolbox $($installed.Version) installed to $destination." -ForegroundColor Green
    Write-Host "Open a new PowerShell session, or run: Import-Module PowerToolbox -Force" -ForegroundColor Cyan
}
catch {
